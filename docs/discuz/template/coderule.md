...
- 程序运行时include嵌套模板内容
	...
	```
### 插件钩子
在模板中设立插件钩子：[插件模板和语言包的设计](/discuz/plugin/language.md)
...
## 插件模板和语言包的设计
**请参见：[插件模板和语言包的设计](/discuz/plugin/language.md)**
## 综合示例
...
<?php
	...
	// 使用自定义模板套系中的forum目录的mytest
	include template('forum/mytest');
?>
```
- 模板代码：
```html
...
- `./template/mytest/forum/mytest`模板代码如下
	...
	<span onclick="changefontsize('{$_G['style']['SMFONTSIZE']}');">改变小号字</span>
	<span onclick="changefontsize('{$_G['style']['THREADTITLEFONTSIZE']}');">改变为大号字</span>
